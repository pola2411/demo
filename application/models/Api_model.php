...
class Api_model extends CI_Model
{
	...
	public function all_silders()
	{
	
		$categories = $this->db->get('sliders')->result_array();
		foreach ($categories as $key => $category) {

			$categories[$key]['thumbnail'] = $this->user_model->get_user_image_url($category['images']);
	...
	public function section_wise_lessons($section_id = "", $user_id = "")
	{
		...
		foreach ($lessons as $key => $lesson) {
			...
			$response[$key]['is_user_buy'] = $user_id;
			...
			$response[$key]['price'] = $lesson['price'];
			$response[$key]['discount_flag'] = $lesson['discount_flag'];
			$response[$key]['is_free_course'] = $lesson['is_free_course'];
	...
	public function course_details_by_id_get($user_id = "", $course_id = "")
	{
		...
		foreach ($response as $key => $resp) {
			$response[$key]['sections'] = $this->sections_get($course_id,$user_id);
			...
			$response[$key]['purchased_all'] = $this->get_active_course_purchases($user_id, $course_id);
	...
	public function get_active_course_purchases($user_id = 0, $course_id = "")
	{
		// 0 represents Not purchased, 1 represents Purchased, 2 represents Pending
		if ($user_id > 0) {
			return get_active_course_purchases($course_id, $user_id);
		} else {
			return 0;
		}
	}
