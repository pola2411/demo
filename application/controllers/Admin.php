...
class Admin extends CI_Controller
{
    ...
    public function slideradd($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add") {
            $this->user_model->add_slider();
            redirect(site_url('admin/sliders'), 'refresh');
        } elseif ($param1 == "edit") {
            $this->user_model->edit_slider($param2);
            redirect(site_url('admin/sliders'), 'refresh');
        } elseif ($param1 == "delete") {
            $this->user_model->delete_slider($param2);
            redirect(site_url('admin/sliders'), 'refresh');
        }

        $page_data['page_name'] = 'slider_add';
        $page_data['page_title'] = get_phrase('slider');
        $this->load->view('backend/index', $page_data);
    }
    
    public function slider_page($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    function server_side_users_data()
    {
        ...
        $columns = array('id', 'id', 'first_name', 'email', 'phone','categoty','level', 'id', 'id');
        ...
        $total_number_of_row = $this->db->where('role_id !=', 1)->where('is_instructor', 0)->get('users')->num_rows();
        ...
        foreach ($students as $key => $student) :
            $category = $this->db->where('id', $student['category_id'])->get('category')->row_array();
            $subcat = $this->db->where('id', $student['subcat'])->get('category')->row_array();
            ...
            $nestedData['category'] = isset($category['name']) ? $category['name'] : '';
            $nestedData['subcat'] = isset($subcat['name']) ? $subcat['name'] : '';
    ...
    public function enrol_section_student($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'enrol') {
            $this->crud_model->enrol_section_student_manually();
            redirect(site_url('admin/enrol_history'), 'refresh');
        }
        $page_data['page_name'] = 'enrol_student';
        $page_data['page_title'] = get_phrase('course_enrolment');
    ...
    public function enrol_lession_student($param1 = "")
    {
        ...
        if ($param1 == 'enrol') {
            $this->crud_model->enrol_lession_student_manually();
            redirect(site_url('admin/enrol_history'), 'refresh');
        }
        $page_data['page_name'] = 'enrol_student';
        $page_data['page_title'] = get_phrase('course_enrolment');
    ...
    public function get_sections() {
        // Retrieve course_id from AJAX request
        $course_id = $this->input->post('course_id');

        // Fetch sections for the selected course
        $sections = $this->db->where('course_id', $course_id)->get('section')->result_array();
        ...
        foreach ($sections as $section) {
            $response[] = array(
                ...
                'title' => $section['title'],
                'price'=>$section['price'],
                'discounted_price'=>$section['discounted_price']
            );
        }
    ...
    public function get_lession() {
        // Retrieve course_id from AJAX request
        $course_id = $this->input->post('course_id');

        // Fetch sections for the selected course
        $sections = $this->db->where('section_id', $course_id)->get('lesson')->result_array();
    ...
    {
        $course_id = $this->input->post('course_id');
        $subcategories =   $this->db->where('parent',$course_id)->get('category')->result_array();
    ...
    public function sliders($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['data'] =  $this->db->get('sliders')->result_array();
    ...
    public function ajax_get_subcat($category_id)
    {
        $page_data['sections'] = $this->crud_model->get_sub_categories($category_id)->result_array();
        return $this->load->view('backend/admin/ajax_get_section', $page_data);
    }
