...
class Api extends REST_Controller {
  ...
  // public function web_redirect_to_buy_course_get($auth_token = "", $course_id = "",$section='',$lesson="", $app_url = "",$type=""){
  //   $section1 = [];
  //   $lession =['1'];
  ...
  //               $lession_details = $this->crud_model->get_lession_by_id($les)->row_array();
  //               if($lession_details['discount_flag'] == 1){
  //                 $price += $lession_details['discounted_price'];
  //               }else{
  //                 $price += $lession_details['price'];
  //               }
  ...
  //               $section_details = $this->crud_model->get_section_by_id($sec)->row_array();
  //               if($section_details['discount_flag'] == 1){
  //                 $price += $section_details['discounted_price'];
  //               }else{
  //                 $price += $section_details['price'];
  //               }
  ...
  //         $this->session->set_userdata('lession', $lession);
  ...
  //   }
  // }
  public function web_redirect_to_buy_course_get($auth_token = "",$app_url = "",$course_id="",$types="")
{
  $course_id = [
    $course_id
  ];
  ...
  $type =$types;
    ...
    $user_id = $user_details['user_id'] ?? null;
    ...
    $row = $query->row();
    $this->initialize_user_session($row, $app_url);
    
    $this->handle_cart_items($course_ids, $type);
...
private function initialize_user_session($user, $app_url)
{
    $this->session->set_userdata([
        'custom_session_limit' => (time() + 604800),
        'user_id' => $user->id,
    ...
    $this->session->set_flashdata('flash_message', 'Welcome ' . $user->first_name . ' ' . $user->last_name);
}
...
private function handle_cart_items($ids, $type)
{
    ...
    foreach ($cart_items['id'] as $id) {
   
        $item_details = $this->fetch_item_details($id, $type[$key]);
        $price += ($item_details['discount_flag'] == 1) ? $item_details['discounted_price'] : $item_details['price'];
    }
    ...
    $this->session->set_userdata([
        'total_price_of_checking_out' => $price,
        'cart_items' => $cart_items,
    ]);
}

private function fetch_item_details($id, $type)
{
    switch ($type) {
        case 'course':
            return $this->crud_model->get_course_by_id($id)->row_array();
        case 'lesson':
            return $this->crud_model->get_lession_by_id($id)->row_array();
        case 'section':
            return $this->crud_model->get_section_by_id($id)->row_array();
        default:
            return []; // Handle unexpected type more gracefully in real scenarios
    }
}
  ...
  public function all_categories_get() {
    ...
    // Fetch all the categories
    public function all_sliders_get() {
      $categories = array();
      $categories = $this->api_model->all_silders();
      $this->set_response($categories, REST_Controller::HTTP_OK);
    }
